...
import { ArrowLeft, Star, Copy, Check, Download } from "lucide-react"; // Import Download icon
import { ChatMessage, ModelId, MODELS } from "@/types/chat";
import MarkdownRenderer from "./MarkdownRenderer";
import ChatInput from "./ChatInput";
import RatingPopup from "./RatingPopup";
import jsPDF from "jspdf"; // Import jsPDF
import autoTable from "jspdf-autotable"; // FIX: Import autoTable as a standalone module
...
const ActiveChat = ({ modelId, messages, onSendMessage, onBack, isLoading }: ActiveChatProps) => {
  ...
  const handleDownloadPDF = (content: string, dateStr?: string | Date) => {
      ...
      // Content Parsing (Basic separation of code blocks and text)
      let yOffset = 38;
      ...
      parts.forEach((part, index) => {
          if (index % 2 !== 0) {
              ...
              // Remove the language identifier (e.g., 'abap') from the first line if present
              const codeContent = codeLines.length > 1 && !codeLines[0].includes(" ") ? codeLines.slice(1).join("\n") : part;
              
              // FIX: Use autoTable as a standalone function
              autoTable(doc, {
                  startY: yOffset,
                  head: [],
                  body: [[codeContent]],
      ...
      // Footer
      // FIX: Cast doc.internal to any to bypass TS compilation error for getNumberOfPages
