  ...
  return (
    <div className={minimal ? "w-full" : "flex flex-col items-center gap-8 w-full max-w-3xl mx-auto"}>
      ...
      <form onSubmit={handleSubmit} className={`w-full ${minimal ? "" : "animate-slide-up"}`}>
        <div className={`relative bg-card border border-border shadow-sm flex flex-col focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/50 transition-all ${minimal ? "rounded-md" : "rounded-lg"}`}>
          <textarea
            value={input}
            ...
            placeholder={placeholder || "Describe your enterprise requirement (e.g., ABAP, Node.js, CAP, Fiori)..."}
            rows={minimal ? 1 : 4}
